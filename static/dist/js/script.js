...
function addToLog(message) {
    const answerBlock = _cloneAnswerBlock()
...
async function readResponseChunks(reader, answerBlock) {
    ...
    let chunks = ""
    while (true) {
        const { done, value } = await reader.read() // done is false while response generation is not over
        if (done) { break }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener("submit", async (event) => {
        ...
        try {
            const answerBlock = addToLog('GPT en en train de reflechir')
